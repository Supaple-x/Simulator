...
void ASimHUD::StartGame(FVector DesiredDroneLocation)
{
    try {
        UAirBlueprintLib::OnBeginPlay();
        initializeSettings();
        setUnrealEngineSettings();
        createSimMode(DesiredDroneLocation);
        createMainWidget();
        setupInputBindings();
        if (simmode_)
...
{
	UCPP_GameInstanceBase* GI = Cast<UCPP_GameInstanceBase>(UGameplayStatics::GetGameInstance(GetWorld()));
	if (GI)
	{
		if (GI->bCharacterIsDrone)
			return "Multirotor";
		else
			return "Car";
	}
...
void ASimHUD::createSimMode(FVector DesiredDroneLocation)
{
